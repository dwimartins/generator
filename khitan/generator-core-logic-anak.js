/**
 * ============================================================================
 * CORE LOGIC GENERATOR UNDANGAN ANAK (Khitan, Aqiqah, Ultah)
 * ============================================================================
 ...
 * 2. Menghindari konflik dengan logic tema Wedding.
 * 3. Memudahkan kustomisasi warna modal peringatan per tema.
 * ============================================================================
 */

window.SistemLogikaAnak = {
    
    // ==========================================
    // MODUL 1: LOGIKA RSVP & KEAMANAN
    // ==========================================
    RSVP: {
        getScript: function(brideId, isDemoMode, isSpamFilterOn, themeColor = 'bg-gray-700') {
            if (isDemoMode) {
                return `
                    ...
                    if(e.target.closest('#btn-preview-kirim')) {
                        ...
                        if(typeof window.showModal === 'function') {
                            window.showModal(
                                ...
                                [{text:"Mengerti", color:"${themeColor} text-white", action:"tutupModal()"}]
                            );
                ...
                document.addEventListener('click', function(e) {
                    if(e.target.closest('#btn-preview-kirim')) {
                        ...
                        if (!namaVal || !ucapanVal || !kehadiranVal) { 
                            window.showModal("Peringatan", "Mohon lengkapi semua kolom form RSVP terlebih dahulu.", [{text:"Mengerti", color:"${themeColor} text-white", action:"tutupModal()"}]); 
                            return; 
                        }
                        
                        if(localStorage.getItem('rsvp_${brideId}')) { 
                            window.showModal("Terima Kasih", "Anda sudah mengirimkan RSVP/Ucapan sebelumnya. 1 perangkat hanya dapat mengirim 1 kali.", [{text:"Tutup", color:"${themeColor} text-white", action:"tutupModal()"}]); 
                            return; 
                        }
                        ...
                        if(containsBadWord || urlRegex.test(ucapanVal)) { 
                            window.showModal("Peringatan Keamanan", "Maaf, ucapan tidak dapat dikirim karena mengandung tautan/link atau kata-kata yang tidak pantas.", [{text:"Ubah Ucapan", color:"${themeColor} text-white", action:"tutupModal()"}]); 
                            return; 
                        }
                        ...
                        window.showModal("Konfirmasi Pengiriman", "Pastikan ucapan dan kehadiran Anda sudah benar. Pesan yang telah dikirim tidak dapat diubah kembali.", [
                            {text:"Periksa Lagi", color:"${themeColor} text-white opacity-80", action:"tutupModal()"},
    ...
    // ==========================================
    Waktu: {
        buildCalendarLink: function(dateIso, timeStart, timeEnd, judulAcara, lokasi) {
            ...
            try {
                ...
                const calendarText = judulAcara; 
                const calendarDetails = `Kami sangat mengharapkan kehadiran Bapak/Ibu/Saudara/i pada acara kami.`;
        ...
        getCountdownScript: function(dateIso, timeStart) {
            ...
            try {
                ...
                return `
                (function(){
                  const daysEl  = document.getElementById('cd-hari') || document.getElementById('cd-days');
                  const hoursEl = document.getElementById('cd-jam') || document.getElementById('cd-hours');
                  const minsEl  = document.getElementById('cd-menit') || document.getElementById('cd-mins');
                  const secsEl  = document.getElementById('cd-detik') || document.getElementById('cd-secs');
    ...
    Interaksi: {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const targetSection = document.getElementById('section-profil') || document.getElementById('section-tujuan'); 
            ...
            const handleFirstClick = (e) => { 
                ...
                setTimeout(() => { scrollKeTujuan(); }, 1200); 
            ...
            const selector = ".muncul, .muncul-kiri, .muncul-kanan, .zoom"; const elementVisible = 100; const qAll = (sel, root=document) => Array.from(root.querySelectorAll(sel));
    ...
    // ==========================================
    // MODUL 4: ASET & DATABASE BANK (WEDDING & KHITAN)
    // ==========================================
    AssetBank: {
    ...
    // ==========================================
    // MODUL 5: ASET MUSIK KHUSUS ANAK (ISLAMI DLL)
    // ==========================================
    AssetMusik: {
        list: [
        ...
        getOptionsHTML: function() {
            let options = '<option value="custom">-- Gunakan Link Custom / Ketik Sendiri --</option>';
